...
async def ping(e):
    if e.sender_id in SUDO_USERS:
        start = datetime.now()
        altron = await e.reply(f"» #​ℂ𝕌𝕋𝔼 𝔹𝕆𝕐 𝕠𝕡 𝕓𝕠𝕝𝕥𝕖", parse_mode=None, link_preview=None)
        end = datetime.now()
        mp = (end - start).microseconds / 1000
        await altron.edit(f"__🤖 ᴘɪɴɢ jai shree ram нαтєяѕ мєяє ℓαυяє ρє​__\n» `{mp} ms`")
...
async def addsudo(event):
    if event.sender_id == OWNER_ID:
        ok = await event.reply(f"» __cute boy ka ek ɴᴀʏᴀ ʙᴇᴛᴀ ᴀᴅᴅ ʜᴏ ʀʜᴀ...__")
        mks = "SUDO_USER"
        target = ""
